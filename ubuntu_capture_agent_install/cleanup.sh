...
SRC_LIST_BKP=/etc/apt/sources.list.backup
# Location of matterhorn files
OC_DIR=/opt/matterhorn
# The directory where the capture agent files live
CA_DIR=/opt/matterhorn/capture-agent
# Path to the matterhorn startup script
STARTUP_SCRIPT=/etc/init.d/matterhorn
# Path to the file specifying rules for the installed devices
RULES_FILE=/etc/udev/rules.d/matterhorn.rules
# List of packages to be uninstalled
PKG_LIST="v4l-conf ivtv-utils curl maven2 sun-java6-jdk openssh-server gstreamer0.10-plugins-ugly gstreamer0.10-plugins-ugly-multiverse gstreamer0.10-ffmpeg ntp gstreamer0.10-plugins-bad gstreamer0.10-plugins-bad-multiverse"
